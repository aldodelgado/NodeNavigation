 ...
 */
class NodeNavigationEventHandler extends Object implements CakeEventListener {
 ...
 */
	public function onLayoutAfterFilter($event) {
		...
		$event->data['content'] = $this->filterChildMenu($event->data['content']);
	}
 ...
 * @return string
 */
	public function childMenu($options = array()) {
		if (!isset($this->view->viewVars['nodeNavigation']['children'])) {
			return false;
		}
		...
		$output = $this->view->element(
			$options['element'], 
			array(
				'links' => $this->view->viewVars['nodeNavigation']['children'],
				'options' => $options,
			), 
			array(
				'plugin' => 'NodeExtras'
...
/**
 * Filter content for ChildMenus
 *
 * Replaces [childmenu:content_type_alias] or [cm:content_type_alias] with NodeTypeArchive
 *
 * @param string $content
 * @return string
 */
	public function filterChildMenu($content) {
		preg_match_all('/\[(childmenu|cm)(.*?)\]/i', $content, $tagMatches);
		for ($i = 0, $ii = count($tagMatches[1]); $i < $ii; $i++) {
			$regex = '/(\S+)=[\'"]?((?:.(?![\'"]?\s+(?:\S+)=|[>\'"]))+.)[\'"]?/i';
			preg_match_all($regex, $tagMatches[2][$i], $attributes);
			$options = array();
			for ($j = 0, $jj = count($attributes[0]); $j < $jj; $j++) {
				$options[$attributes[1][$j]] = $attributes[2][$j];
			}
			$content = str_replace($tagMatches[0][$i], $this->childMenu($options), $content);
		}
		return $content;
	}
