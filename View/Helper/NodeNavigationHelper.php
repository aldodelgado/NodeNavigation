 ...
 */
	public function breadcrumbs($path = array()) {
	  ...
	  return $this->_View->element('NodeNavigation.breadcrumbs');
	}
 ...
 */
	public function submenu($submenu = array()) {
  	if($submenu['parentId'] && !empty($submenu['links'])) {
  	  if($this->_View->Layout->node('id')) {
  	    return $this->_View->element('NodeNavigation.submenu', array('links'=>$submenu['links']));
  	  }
  	}
	  return false;
	}
